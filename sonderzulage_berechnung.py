...
def format_date_with_weekday_and_week(date):
    if pd.isnull(date):
        return ""
    weekday = date.strftime("%A")  # Wochentag
    week_number = date.isocalendar()[1]  # Kalenderwoche
    formatted_date = date.strftime(f"%d.%m.%Y({weekday}, KW{week_number})")
...
def main():
    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        ...
        for year, month in sorted_data[["Jahr", "Monat"]].drop_duplicates().values:
            ...
            try:
                ...
                sheet_name = "".join(char if char.isalnum() or char.isspace() else "_" for char in month_name)[:31]
            except KeyError:
                sheet_name = f"Unbekannter_Monat_{year}"
    ...
    writer.book.save(output_file)  # Speichern des Workbooks sicherstellen
